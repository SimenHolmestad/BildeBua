...
Hvis du vet at noen (eller alle) av photobooth-brukerne ikke er koblet til WiFi-nettverket ditt, kan det være lurt å gi dem en QR-kode for å koble seg til. Å lage en slik QR-kode kan gjøres enkelt med BildeBua.

For å lage en WiFi-QR-kode må du først konfigurere WiFi-innstillingene i `.env`. Sett følgende variabler:

```
BILDEBUA_WIFI_QR_CODE__ENABLED=true
BILDEBUA_WIFI_QR_CODE__WIFI_NAME=my_wifi_SSID
BILDEBUA_WIFI_QR_CODE__PROTOCOL=WPA/WPA2
BILDEBUA_WIFI_QR_CODE__PASSWORD=my_super_secret_password
BILDEBUA_WIFI_QR_CODE__DESCRIPTION=Scan qr code to connect to my_wifi_SSID!
```
