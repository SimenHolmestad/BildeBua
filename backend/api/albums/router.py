...
from backend.camera.modules.base_camera_module import ImageCaptureError
...
class ForcedAlbumResponse(BaseModel):
    forced_album: str
...
def construct_album_api_router(
    ...
    @album_api_router.get("/", response_model=Union[AvailableAlbumsResponse, ForcedAlbumResponse])
    ...
    @album_api_router.api_route(
        "/{album_name}",
        methods=["GET", "POST"],
        name="album_info",
        response_model=Union[AlbumInfoResponse, AlbumCaptureResponse]
    )
